...
struct WindowBuildOptions<'a> {
    ...
    /// True when `url` is the first-run "who does this computer belong to?"
    /// prompt rather than the real target page. Gates the last-url restore,
    /// the macOS auth/cert-bypass bootstrap, and the on_navigation handler
    /// that captures the submitted name (see `app::device_name`).
    awaiting_device_name: bool,
}
...
fn build_window_with_label(
    ...
    // Only the main window asks; a stored name is required before any
    // "web" content (including multi-window clones) loads with it attached.
    let stored_device_name = if window_config.url_type == "web" {
        crate::app::device_name::read_stored_device_name(app)
    } else {
        None
    };
    let awaiting_device_name =
        label == "pake" && window_config.url_type == "web" && stored_device_name.is_none();

    let url = if awaiting_device_name {
        WebviewUrl::CustomProtocol(crate::app::device_name::prompt_page_url())
    } else {
        match window_config.url_type.as_str() {
            "web" => {
                let effective_url = match &stored_device_name {
                    Some(name) => {
                        crate::app::device_name::append_device_name_query(&window_config.url, name)
                    }
                    None => window_config.url.clone(),
                };
...
fn build_window(
    ...
    if awaiting_device_name {
        // Capture the prompt page's `pake-device-name:set?name=...` handoff
        // (see device_name_prompt.html), save it, then load the real target
        // with `deviceName` attached. Deferred to a spawned task rather than
        // handled inline: this closure is itself a navigation decision, and
        // navigating the same window from inside it risks the kind of
        // webview-callback reentrancy documented for on_download above.
        let device_name_app = app.clone();
        let device_name_label = label.to_string();
        let device_name_target = window_config.url.clone();
        window_builder = window_builder.on_navigation(move |nav_url| {
            let Some(name) = crate::app::device_name::extract_submitted_device_name(nav_url)
            else {
                return true;
            };

            let app_handle = device_name_app.clone();
            let label = device_name_label.clone();
            let base_url = device_name_target.clone();
            tauri::async_runtime::spawn(async move {
                if let Err(error) = crate::app::device_name::write_device_name(&app_handle, &name)
                {
                    eprintln!("[Pake] Failed to save the device name: {error}");
                }

                let target = crate::app::device_name::append_device_name_query(&base_url, &name);
                match Url::parse(&target) {
                    Ok(parsed) => {
                        if let Some(window) = app_handle.get_webview_window(&label) {
                            if let Err(error) = window.navigate(parsed) {
                                eprintln!(
                                    "[Pake] Failed to navigate after saving the device name: {error}"
                                );
                            }
                        }
                    }
                    Err(error) => {
                        eprintln!("[Pake] Failed to build the target URL with the device name: {error}");
                    }
                }
            });
