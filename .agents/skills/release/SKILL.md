...
1. [ ] Before changing files or running any formatter/build, freeze the full baseline with `git status --short --branch -uall`, `git diff`, and `git diff --cached`. If the checkout contains unrelated or ambiguously owned changes, prepare the release in an isolated worktree instead of letting a write-capable check touch the live checkout.
2. [ ] Confirm the new version number (check current: `cat package.json | jq .version`; previous tag: `git tag --list 'V*' --sort=-version:refname | head -1`; a bare `git tag --sort` picks up stray non-version tags like `list` and `continuous`)
3. [ ] Confirm the version is not already on npm: `npm view pake-cli@X.Y.Z version` should return 404 before publishing
4. [ ] Update all four version files above
5. [ ] Run `pnpm run format`, must pass cleanly
6. [ ] Run `pnpm test`, must pass cleanly. If this step fails at the suite's `pnpm install pake-cli@latest` (run in `/tmp`) with exit code 1 against a mirror, re-run once; a single transient flake is acceptable, two consecutive failures is not.
7. [ ] Run `pnpm run cli:build`, Rollup + TS must pass (catches type errors that `format` misses).
8. [ ] Run `pnpm run release:check`, verifies version sync, package contents, and npm dry-run
9. [ ] Re-read `git status --short --branch -uall`, `git diff`, and `git diff --cached`. Restore only known build-churn paths that were clean in the frozen baseline and whose new diff is provably generated by this run. If ownership is ambiguous, leave the live checkout untouched and repeat the release preparation in an isolated worktree; do not absorb unrelated churn into the release commit.
10. [ ] Commit version bump with message: `chore: bump version to VX.X.X`. Include the rebuilt `dist/cli.js` (it embeds the version); stage it with `git add -f dist/cli.js` since `dist/` is gitignored.
...
- npm scans newly published packages before making them installable, typically for about five minutes and sometimes over 15 minutes ([npm announcement](https://github.blog/changelog/2026-07-28-npm-publish-time-malware-scanning-and-dual-use-metadata/)). The workflow allows 20 minutes for visibility. A successful publish followed by a verification timeout does not authorize republishing: inspect the registry and install the exact version once it becomes available, then report publish and verification outcomes separately.
